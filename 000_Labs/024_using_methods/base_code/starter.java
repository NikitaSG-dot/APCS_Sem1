...
class starter {
	...
	public static void main(String args[]) throws InterruptedException {
		...
		while (true)
		{
			System.out.println("Would you like to play a random sentence [1], complete a set sentence [2], or quit [3]?");
			System.out.print(getTime());
			System.out.println(" is the current time.");
			choice = sc.nextInt();
			sc.nextLine();
			if (choice == 1)
			{
				...
				time = endTimer();
				System.out.println("You have "+compareSentences(give, type)+" errors out of "+lettersInSentence(give)+" letters.");
				System.out.println((int)((wordsInSentence(give))/(time/60))+" words per minute.");
				System.out.println((int)((lettersInSentence(give))/(time/60))+" letters per minute.");
				
			}
			else if (choice == 2)
			{
				System.out.println("From 1-100, what sentence would you like to play?");
				sent = sc.nextInt();
				sc.nextLine();
				System.out.println("---------------------------------------------------------");
				give = getSentence(sent);
				System.out.println(give);
				System.out.println("Get ready!");
				countdown();
				startTimer();
			...
			{
				System.out.println("That was not one of the options. Please input 1,2, or 3.");
			}
		}
	}
